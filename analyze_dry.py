...
def analyze_dry():
    ...
    ax1.plot(psd.freqs, psd_data.mean(axis=0), color='#e67e22', linewidth=2.5)
    ...
    im3, _ = mne.viz.plot_topomap(noise_data, raw.info, axes=ax3, cmap='YlOrBr', show=False)
    ...
    ax4.bar(range(len(snr_per_ch)), snr_per_ch, color='#e67e22')
    ...
    plt.suptitle("DRY-BASED QUALITY REPORT: waveguard™ touch (CW-06879)", fontsize=20, fontweight='bold')
    plt.show()
